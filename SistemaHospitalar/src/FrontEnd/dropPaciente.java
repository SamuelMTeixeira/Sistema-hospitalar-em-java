 ...
 * @author samuel
 */
public class dropPaciente extends javax.swing.JFrame {
    ...
    private void setAtualizarGrid (String filtro){
        ...
        try {
            ...
            String SQL = "SELECT p.nome as nome, p.CPF AS CPF FROM paciente p ORDER BY "+filtro;
            ...
            if (this.conexao.getResultSet().first() ){
                do {
                    dtm.addRow  (
                            new Object[] {
                                this.conexao.getResultSet().getString("p.nome"),
                                this.conexao.getResultSet().getString("p.CPF"),
                            }
                    );
    ...
    private void setDeleteGrid (String cpfPaciente ) {      
    String codigo = null;
        try {
            this.conexao.setResultSet("SELECT codPaciente FROM paciente WHERE CPF = '"+cpfPaciente);
            if(this.conexao.getResultSet().first() )
                    codigo = this.conexao.getResultSet().getString("codPaciente");
                    this.conexao.SQLExecute("DELETE FROM listaespera WHERE codPaciente = "+codigo);
                    this.conexao.SQLExecute("DELETE FROM paciente WHERE codPaciente = "+codigo);
            }
            catch (SQLException e){
    ...
    private void setAtualizarSalaEspera (String filtro){
    ...
    try {
            this.conexao.setResultSet("SELECT codPaciente FROM paciente WHERE CPF = '");
            if(this.conexao.getResultSet().first() )
                    this.conexao.getResultSet().getString("codPaciente");
    ...
    public dropPaciente(JTable grd) {
        initComponents();
    ...
    private void initComponents() {
        ...
        txtExcluirPaciente.setText("EXCLUIR PACIENTE SELECIONADO");
